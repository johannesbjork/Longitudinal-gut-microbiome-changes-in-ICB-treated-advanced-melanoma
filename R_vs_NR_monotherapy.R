...
plot_RvsNR_monotherapy <- function(pibble_fit=pibble_fit, focus_var=focus_var, siglevel=siglevel) {
  ...
  intercepts_and_slopes <- 
    ...
    mutate(
      ## Intercepts
      # Z=0; W=0 (PFS12=0; e.g. W1=0)
      intercept1a = b0,  
      # Z=1; W=0 (PFS12=1; e.g. W1=0)
      intercept1b = b0 + !!rlang::sym(focus_vars[2]),
      
      ## Slopes without intercepts
      # Z=0; W=0 (PFS12=0; e.g. W1=0)
      slope1a = !!rlang::sym(focus_vars[1]), 
      # Z=1; W=0 (PFS12=1; e.g. W1=0)
      slope1b = !!rlang::sym(focus_vars[1]) + !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])), 
      
      # Z=1 vs Z=0 for W=0 (e.g. W1=0)
      avgdiff1 = !!rlang::sym(focus_vars[2]) +
        !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])),
      avgdiff1_x_t0 = 
        !!rlang::sym(paste0(focus_vars[2])) +  
        !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])) * 0, # at t0 (visit=0)
      avgdiff1_x_t1 = 
        !!rlang::sym(paste0(focus_vars[2])) + 
        !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])) * 1, # t1 (visit=1)
      avgdiff1_x_t2 = 
        !!rlang::sym(paste0(focus_vars[2])) + 
        !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])) * 2, # t2 (visit=2)
      avgdiff1_x_t3 = 
        !!rlang::sym(paste0(focus_vars[2])) +  
        !!rlang::sym(paste0(focus_vars[1],":",focus_vars[2])) * 3 # at t3 (visit=3)
    ) %>%  
  ...
  names(ranks_ls) <- c("avgdiff1_x_t0","avgdiff1_x_t1","avgdiff1_x_t2","avgdiff1_x_t3")
  ...
  names(ranks_ls2) <- c("slope1a","slope1b")
  
  included_taxa <- c(ranks_ls$avgdiff1_x_t0$sig_increasing, ranks_ls$avgdiff1_x_t0$sig_decreasing,
                     ranks_ls$avgdiff1_x_t1$sig_increasing, ranks_ls$avgdiff1_x_t1$sig_decreasing,
                     ranks_ls$avgdiff1_x_t2$sig_increasing, ranks_ls$avgdiff1_x_t2$sig_decreasing,
                     ranks_ls$avgdiff1_x_t3$sig_increasing, ranks_ls$avgdiff1_x_t3$sig_decreasing)
  ...
  shared_t1_t0_a <- Reduce(intersect, list(c(ranks_ls$avgdiff1_x_t0$sig_increasing, ranks_ls$avgdiff1_x_t0$sig_decreasing),
                                           c(ranks_ls$avgdiff1_x_t1$sig_increasing, ranks_ls$avgdiff1_x_t1$sig_decreasing)))
  
  # Features shared between t2 and t1
  shared_t1_t2_a <- Reduce(intersect, list(c(ranks_ls$avgdiff1_x_t1$sig_increasing, ranks_ls$avgdiff1_x_t1$sig_decreasing),
                                           c(ranks_ls$avgdiff1_x_t2$sig_increasing, ranks_ls$avgdiff1_x_t2$sig_decreasing)))
  
  # Features shared between t3 and t2
  shared_t2_t3_a <- Reduce(intersect, list(c(ranks_ls$avgdiff1_x_t2$sig_increasing, ranks_ls$avgdiff1_x_t2$sig_decreasing),
                                           c(ranks_ls$avgdiff1_x_t3$sig_increasing, ranks_ls$avgdiff1_x_t3$sig_decreasing)))
  ...
  y_labels_t1_a <- setNames(rep("black",length(c(ranks_ls$avgdiff1_x_t1$sig_increasing, ranks_ls$avgdiff1_x_t1$sig_decreasing))),
                            rev(c(rev(ranks_ls$avgdiff1_x_t1$sig_increasing), rev(ranks_ls$avgdiff1_x_t1$sig_decreasing))))
  
  y_labels_t2_a <- setNames(rep("black",length(c(ranks_ls$avgdiff1_x_t2$sig_increasing, ranks_ls$avgdiff1_x_t2$sig_decreasing))),
                            rev(c(rev(ranks_ls$avgdiff1_x_t2$sig_increasing), rev(ranks_ls$avgdiff1_x_t2$sig_decreasing))))
  
  y_labels_t3_a <- setNames(rep("black",length(c(ranks_ls$avgdiff1_x_t3$sig_increasing, ranks_ls$avgdiff1_x_t3$sig_decreasing))),
                            rev(c(rev(ranks_ls$avgdiff1_x_t3$sig_increasing), rev(ranks_ls$avgdiff1_x_t3$sig_decreasing))))
  ...
  names(plot_ls) <- c("slope1a","slope1b","avgdiff1_x_t0","avgdiff1_x_t1","avgdiff1_x_t2","avgdiff1_x_t3")
...
avg_RvsNR_monotherapy_p90 <- plot_RvsNR_monotherapy(pibble_fit=fit_species_clr, focus_var="visit:PFS12:combiIO", siglevel="p90")
